...
import com.ateliopti.lapplicationpti.futuremind.recyclerviewfastscroll.FastScroller;
...
class CountryCodeDialog {
    ...
    public static void openCountryCodeDialog(final com.ateliopti.lapplicationpti.hbb20.CountryCodePicker codePicker) {
        openCountryCodeDialog(codePicker, null);
    }

    public static void
    openCountryCodeDialog(final com.ateliopti.lapplicationpti.hbb20.CountryCodePicker codePicker, final String countryNameCode) {
        context = codePicker.getContext();
        dialog = new Dialog(context);
        codePicker.refreshCustomMasterList();
        codePicker.refreshPreferredCountries();
        List<com.ateliopti.lapplicationpti.hbb20.CCPCountry> masterCountries = com.ateliopti.lapplicationpti.hbb20.CCPCountry.getCustomMasterCountryList(context, codePicker);
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE);

        // Récupère le type d'écran
        boolean typeEcran = ((MainActivity) context).getTypeEcran();

        if(typeEcran) { // true, petit écran eblaze
            dialog.getWindow().setContentView(R.layout.eblaze_layout_picker_dialog);
